    ...
    if chat.id != Config.OWNER_ID:
        ...
        if customstrmsg is not None:
            buttons = [
                ...
                (Button.url("🔱 Support 🔱", "https://t.me/dominator_bot_support"),),
            ]
            ...
            start_msg = f"Hey! 👤{mention},\nI am {my_mention}'s assistant bot.\nYou can contact to my master from here.\n\nPowered by [DominatorBot](https://t.me/dominator_bot_support)"
            buttons = [
                (
                    Button.inline("🔰 Rules 🔰", data="rules"),
                    Button.inline("❤ Deploy ❤", data="depy"),
        ...
        buttons = [
            (
                Button.url(" Support ", "https://t.me/dominator_bot_support"),
                Button.url(" Updates ", "https://t.me/dominator_bot_official"),
            ),
            (
...
@tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"depy")))
async def help(event):
    if event.query.user_id == bot.uid:
        await event.answer("This Is Not For U My Master", cache_time=0, alert=True)
    else:
        await event.client.send_message(
            event.chat_id,
            message="You Can Deploy/Install DominatorBot In Heroku By Following Steps Bellow, You Can See Some Quick Guides On Support Channel Or On Your Own Assistant Bot. \nThank You For Contacting Me.",
            link_preview=False,
            buttons=[
                [
                ...
                [
                    Button.url(
                        "Github Repo ❓", "https://github.com/DOMINATOR-XD/DOMINATORXBOT"
                    ),
                ],
            ],
        )
...
@DominatorBot_cmd(incoming=True, func=lambda e: e.is_private)
...
@DominatorBot_cmd(pattern="^/uinfo$", from_users=Config.OWNER_ID)
async def bot_start(event):
    reply_to = await reply_id(event)
    if not reply_to:
...
async def send_flood_alert(user_) -> None:
    ...
    if found:
        ...
        try:
            ...
            if BOTLOG:
                await tgbot.send_message(BOTLOG_CHATID, "**Unblock your bot !**")
...
@DominatorBot_cmd(incoming=True, func=lambda e: e.is_private)
@DominatorBot_cmd(edited=True, func=lambda e: e.is_private)
