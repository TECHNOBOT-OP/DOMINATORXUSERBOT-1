...
PATH = "./DominatorBot/cache/ytsearch.json"
menu_category = "bot"
...
async def iytdl_inline(event):
    ...
    domnevent = await eor(event, f"🔎 Searching Youtube for: `'{input_url}'`")
    type = True
    cout = 0
    results = None
    while type:
        ...
        if cout > 5:
            type = False
    if results:
        await domnevent.delete()
        await results[0].click(event.chat_id, reply_to=reply_to_id, hide_via=True)
    else:
        await domnevent.edit("`Sorry!. Can't find any results`")
