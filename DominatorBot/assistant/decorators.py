...
def DominatorBot_cmd(add_cmd, is_args=False):
    def cmd(func):
        DominatorBot = bot.tgbot
        if is_args:
            ...
            pattern = bothandler + add_cmd + "$"
        DominatorBot.add_event_handler(
...
def is_admin():
    def decorator(func):
        ...
        async def wrapper(event):
            DominatorBot = bot.tgbot
            perms = await DominatorBot.get_permissions(event.chat_id, event.sender_id)
...
def is_bot_admin():
    def decorator(func):
        ...
        async def wrapper(event):
            DominatorBot = bot.tgbot
            boat = await DominatorBot.get_me()
            perms = await DominatorBot.get_permissions(event.chat_id, boat)
            if perms.is_admin:
                await func(event)
            else:
