...
@dominator_cmd(pattern="climate ([\s\S]*)")
async def get_weather(weather):
    ...
    await eor(
        ...
        + f"`{cityname}, {fullc_n}`\n"
        + f"`{time}`\n\n"
        + f"**By :**  {dominator_mention}",
    )


@dominator_cmd(pattern="setcity(?:\s|$)([\s\S]*)")
@errors_handler
...
@dominator_cmd(pattern="wttr ([\s\S]*)")
async def _(event):
    global DEFCITY
    reply_to_id = None
    ...
    if not input_str:
        input_str = DEFCITY
    dominator = await eor(event, "Collecting Weather Reports...")
    async with aiohttp.ClientSession() as session:
        sample_url = "https://wttr.in/{}.png"
        response_api_zero = await session.get(sample_url.format(input_str))
        response_api = await response_api_zero.read()
        with io.BytesIO(response_api) as out_file:
            await event.client.send_message(f"**City :** `{input_str}` \n**By :** {dominator_mention}", file=out_file, reply_to=reply_to_id)
    try:
