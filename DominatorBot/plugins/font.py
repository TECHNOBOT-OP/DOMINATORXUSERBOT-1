...
@dominator_cmd(pattern="font(?:\s|$)([\s\S]*)")
async def font(event):
    dominator = await eor(event, "Changing font...")
    flag = event.text[6:]
    rply = await event.get_reply_message()
    if not rply:
        return await eod(dominator, "Nothing given to change!")
    old = rply.text
    normie = normal_str.split(" ")
    prev = " ".join(old).upper()
    if not flag.isnumeric():
        return await eod(dominator, "Give font numbers only!")
    match int(flag):
        case 1:
            to_ = one.split(" ")
            for normal in prev:
        ...
        case 23:
            ...
            await dominator.edit(prev)
        case _:
            await eod(dominator, "Unsupported font!")
