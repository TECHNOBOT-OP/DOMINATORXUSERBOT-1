...
async def _(event):
    ...
    if user.first_name.startswith(OFFLINE_TAG):
        ...
        return
    dominator = await eor(event, "**Changing Profile to Offline...**")
    ...
    if photo:
        ...
        try:
            ...
            await eod(dominator, str(e))
        else:
            await eod(dominator, "**Changed profile to OffLine.**")
    try:
    ...
    try:
        ...
        await eod(dominator, result)
    except Exception as e:
        await eod(dominator, str(e))


@Dominator_cmd(pattern="online$")
async def _(event):
    user_it = "me"
    user = await event.client.get_entity(user_it)
    if user.first_name.startswith(OFFLINE_TAG):
    ...
    if photo:
        ...
        try:
            ...
            await eod(dominator, "**Changed profile to Online.**")
    try:
        os.system("rm -fr donottouch.jpg")
    except Exception as e:
        LOGS.warn(str(e))
...
@Dominator_cmd(pattern="pbio(?:\s|$)([\s\S]*)")
async def _(event):
    bio = event.text[6:]
    try:
        await event.client(
...
@Dominator_cmd(pattern="pname(?:\s|$)([\s\S]*)")
async def _(event):
    names = event.text[7:]
    first_name = names
    last_name = ""
...
@Dominator_cmd(pattern="ppic$")
async def _(event):
    reply_message = await event.get_reply_message()
    dominator = await eor(event, "Downloading Profile Picture to my local ...")
    if not os.path.isdir(Config.TMP_DOWNLOAD_DIRECTORY):
        os.makedirs(Config.TMP_DOWNLOAD_DIRECTORY)
    photo = None
    try:
        ...
        if photo:
            await dominator.edit("Uploading profile picture...")
            file = await event.client.upload_file(photo)
            try:
                await event.client(functions.photos.UploadProfilePhotoRequest(file))
            except Exception as e:
                ...
                await eod(dominator, PP_CHANGED)
    try:
        os.remove(photo)
    except Exception as e:
        LOGS.warn(str(e))
...
@Dominator_cmd(pattern="username(?:\s|$)([\s\S]*)")
...
async def count(event):
    ...
    await dominator.edit(result, parse_mode="HTML")


@Dominator_cmd(pattern="delpfp$")
...
@Dominator_cmd(pattern="myusernames$")
async def _(event):
...
CmdHelp("profile").add_command(
  ...
  "pbio", "<text>", "Changes your Telegram bio", ".pbio dominatoro there, This iz my bio"
).add_command(
