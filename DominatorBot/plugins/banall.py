...
@dominator_cmd(pattern="kickall$", allow_sudo=False)
async def _(event):
    ...
    dominator = await eor(event, "**Bleck Magik Started...**")
...
@dominator_cmd(pattern="banall$", allow_sudo=False)
async def _(event):
    result = await event.client(
        functions.channels.GetParticipantRequest(event.chat_id, event.client.uid)
    )
    ...
    dominator = await eor(event, "**Bleck Magik Begins..**")
    admins = await event.client.get_participants(
        event.chat_id, filter=ChannelParticipantsAdmins
    )
    admins_id = [i.id for i in admins]
...
@dominator_cmd(pattern="unbanall$")
async def _(event):
    if event.is_private:
        return
...
@dominator_cmd(pattern="ikuck(?:\s|$)([\s\S]*)")
async def _(event):
    if event.is_private:
        return
    ...
    q = 0
    r = 0
    dominator = await eor(event, "Searching Participant Lists.")
    async for i in event.client.iter_participants(event.chat_id):
        ...
        if isinstance(i.status, UserStatusEmpty):
            ...
            if "y" in input_str:
                ...
                if not status:
                    await eod(dominator, "I need admin priveleges to perform this action!")
                    e.append(str(e))
                    break
                else:
                    c = c + 1
...
Bots: {}
None: {}"""
        await dominator.edit(required_string.format(c, p, d, y, m, w, o, q, r, b, n))
