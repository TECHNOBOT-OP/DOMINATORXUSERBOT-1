...
DominatorBot_info = "https://raw.githubusercontent.com/DominatorBot/Plugins/master/DominatorBot-info.json"
urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)
requirements_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "requirements.txt")


async def dominator_info(DominatorBot_info):
    infos = requests.get(DominatorBot_info).json()
    _version = infos['DominatorBot-INFO']['version']
    _release = infos['DominatorBot-INFO']['release-date']
    _branch = infos['DominatorBot-INFO']['branch']
    _author = infos['DominatorBot-INFO']['author']
    _auturl = infos['DominatorBot-INFO']['author-url']
...
async def update(event, repo, ups_rem, ac_br):
    ...
    await eor(event, "✅ Successfully updated Dominator Bot!\n\nBot is restarting please wait for a minute.")
    args = [sys.executable, "-m", "DominatorBot"]
    os.execle(sys.executable, *args, os.environ)
    return


@dominator_cmd(pattern="update(| now)$")
async def upstream(event):
    ...
    if changelog == "" and not force_update:
        _version, _release, _branch, _author, _auturl = await dominator_info(DominatorBot_info)
        output_ = f"**Your Bot Version :** `{dominator_ver}` \n**Owner :** {dominator_mention} \n\n**Official DominatorBot Version :** `{_version}` \n**Release Date :** `{_release}` \n**Official Repo Branch :** `{_branch}` \n**Update By :** [{_author}]({_auturl})"
        if str(_version) not in str(dominator_ver):
            output_ += f"\n\n**Do** `{hl}update build` **to update your DominatorBot to latest version.**"
        await event.edit(output_)
        return repo.__del__()
    if conf == "" and not force_update:
        await print_changelogs(event, ac_br, changelog)
        await event.delete()
        return await event.respond(f"🌚 Do `{hl}update build` to update your **DominatorBot** !!")

    if force_update:
        await event.edit(
            "`Force-Updating Dominator bot. Please wait...`"
        )
    if conf == "now":
...
async def deploy(event, repo, ups_rem, ac_br, txt):
    if HEROKU_API_KEY is not None:
        ...
        await event.edit(f"**Your DominatorBot Is UpToDate**\n\n**Version :**  __{dominator_ver}__\n**Oɯɳҽɾ :**  {dominator_mention}")
    else:
        await event.edit("**Please set up**  `HEROKU_API_KEY`  **from heroku to update!**")
    return


@dominator_cmd(pattern="update build$")
async def upstream(event):
    event = await eor(event, "`Hard-Update In Progress... \nPlease wait until docker build is finished...`")
    off_repo = "https://github.com/dominator454/DOMINATORXUSERBOT"
    os.chdir("/app")
    git_dominator = f"rm -rf .git"
    try:
        await runcmd(git_dominator)
    except BaseException:
        pass
    txt = "😕 `Updater cannot continue due to some problems occured`\n\n**LOGTRACE:**\n"
    try:
...
CmdHelp("update").add_command(
  ...
  "update now", None, "Soft-Update Your DominatorBot. Basically if you restart dyno it will go back to previous deploy."
).add_command(
  "update build", None, "Hard-Update Your Dominatorbot. This won't take you back to your previous deploy. This will be triggered even if there is no changelog."
).add_info(
  "DominatorBot Updater."
).add_warning(
  "✅ Harmless Module."
).add()
