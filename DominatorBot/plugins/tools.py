...
@Dominator_cmd(pattern="scan(?:\s|$)([\s\S]*)")
async def _(event):
    ...
    if not reply_message.media:
        ...
        return
    chat = "@DrWebBot"
    dominatorevent = await eor(event, " `Scanning This media..... wait👀`")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await eod(dominatorevent, "`Please unblock `@DrWebBot `and try again`")
            return
        if response.text.startswith("Forward"):
            await eod(dominatorevent,
...
@Dominator_cmd(pattern="barcode(?:\s|$)([\s\S]*)")
async def _(event):
    dominatorevent = await eor(event, "...")
    ...
    try:
        ...
        await dominatorevent.edit(str(e))
        return
    end = datetime.datetime.now()
    ms = (end - start).seconds
    await dominatorevent.edit("Created BarCode in {} seconds🤓".format(ms))
    await asyncio.sleep(5)
    await dominatorevent.delete()


@Dominator_cmd(pattern="makeqr(?: |$)([\s\S]*)")
...
@Dominator_cmd(pattern="cal(?:\s|$)([\s\S]*)")
async def _(event):
    input_str = event.text[5:]
    dominator = await eor(event, "Processing...")
    ...
    if len(input_sgra) == 3:
        ...
        await dominator.edit(str(a))
    else:
        await eod(dominator, f"SYNTAX: {hl}calendar YYYY.MM.DD")


@Dominator_cmd(pattern="currency(?:\s|$)([\s\S]*)")
async def _(event):
    input_str = event.pattern_match.group(1)
    input_sgra = input_str.split(" ")
    if len(input_sgra) == 3:
...
@Dominator_cmd(pattern="currencies$")
...
@Dominator_cmd(pattern="ifsc(?:\s|$)([\s\S]*)")
async def _(event):
    input_str = event.pattern_match.group(1)
...
@Dominator_cmd(pattern="color(?:\s|$)([\s\S]*)")
async def _(event):
    input_str = event.pattern_match.group(1)
    message_id = None
...
@Dominator_cmd(pattern="xkcd(?:\s|$)([\s\S]*)")
async def _(event):
    dominatorevent = await eor(event, "`processiong...`")
    input_str = event.pattern_match.group(1)
    xkcd_id = None
    if input_str:
...
Year: {}""".format(
        ...
        await dominatorevent.edit(output_str, link_preview=True)
    else:
        await eod(dominatorevent, "xkcd n.{} not found!".format(xkcd_id))

@Dominator_cmd(pattern="dns(?:\s|$)([\s\S]*)")
async def _(event):
    input_str = event.pattern_match.group(1)
...
@Dominator_cmd(pattern="url(?:\s|$)([\s\S]*)")
async def _(event):
    input_str = event.pattern_match.group(1)
...
@Dominator_cmd(pattern="unshort(?:\s|$)([\s\S]*)")
async def _(event):
    input_str = event.pattern_match.group(1)
...
CmdHelp("tools").add_command(
  ...
  "currency", "<amount> <from> <to>", "Currency converter for DominatorBot", ".currency 10 usd inr"
).add_command(
