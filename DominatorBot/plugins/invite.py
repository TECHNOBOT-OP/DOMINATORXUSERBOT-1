...
@Dominator_cmd(pattern="inviteall(?:\s|$)([\s\S]*)")
async def get_users(event):
    hel_ = event.text[11:]
    dominator_chat = hel_.lower()
    restricted = ["@DominatorBot_chat", "@its_DominatorBot"]
    dominator = await eor(event, f"__Inviting members from__ {hel_}")
    if dominator_chat in restricted:
        await dominator.edit("You can't Invite Members from there.")
        await event.client.send_message(-1001496036895, "Sorry for inviting members from here.")
        return
    kraken = await get_chatinfo(event)
    chat = await event.get_chat()
    if event.is_private:
        return await dominator.edit("`Sorry, Cant add users here`")
    s = 0
    f = 0
    error = "None"
    await dominator.edit("**INVITING USERS !!**")
    async for user in event.client.iter_participants(kraken.full_chat.id):
        try:
            ...
            error = str(e)
            f += 1
    return await dominator.edit(
        f"**INVITING FINISHED** \n\n**Invited :**  `{s}` users \n**Failed :**  `{f}` users."
    )


@Dominator_cmd(pattern="add(?:\s|$)([\s\S]*)")
async def _(event):
    if (
