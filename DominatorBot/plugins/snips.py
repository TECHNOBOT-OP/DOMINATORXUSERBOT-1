...
from DominatorBot.sql import snip_sql as sq
from . import *


@dominator_cmd(pattern=r'\#(\S+)')
...
@dominator_cmd(pattern="snip(?:\s|$)([\s\S]*)")
...
@dominator_cmd(pattern="rmsnip(?:\s|$)([\s\S]*)")
async def _(event):
    lg_id = Config.LOGGER_ID
...
@dominator_cmd(pattern="listsnip$")
