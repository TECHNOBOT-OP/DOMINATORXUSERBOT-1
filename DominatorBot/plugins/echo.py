...
from DominatorBot.sql.echo_sql import addecho, get_all_echos, is_echo, remove_echo
from DominatorBot.sql.gvar_sql import addgvar, gvarstat, delgvar
from . import *


@Dominator_cmd(pattern="echo$")
...
@Dominator_cmd(pattern="listecho$")
async def echo(event):
    lsts = get_all_echos()
    if len(lsts) > 0:
