...
from DominatorBot.sql.fsub_sql import *
from . import *
...
@dominator_cmd(pattern="fsub(?:\s|$)([\s\S]*)")
async def _(event):
    if event.is_private:
...
async def _(event):
    ...
    if uid == event.sender_id:
        ...
        try:
            ...
            print(str(e))
            return
        msg = f"**dominatoro {nm} !! Welcome to {(await event.get_chat()).title} ✨**"
