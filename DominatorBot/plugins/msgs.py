...
async def all_groups_id(dominator):
    dominatorgroups = []
    async for dialog in dominator.client.iter_dialogs():
        entity = dialog.entity
        if isinstance(entity, Channel) and entity.megagroup:
            dominatorgroups.append(entity.id)
    return dominatorgroups


@dominator_cmd(pattern="frwd$")
async def _(event):
...
@dominator_cmd(pattern="fpost(?:\s|$)([\s\S]*)")
async def _(event):
    global groupsid
    global msg_cache
    await event.delete()
