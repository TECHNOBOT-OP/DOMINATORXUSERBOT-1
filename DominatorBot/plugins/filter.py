...
@dominator_cmd(pattern="filter(?:\s|$)([\s\S]*)")
async def on_snip_save(event):
    name = event.pattern_match.group(1)
    msg = await event.get_reply_message()
    ForGo10God, _, dominator_mention = await client_id(event)
    _id = event.chat_id
    if msg:
...
@dominator_cmd(pattern="stop(?:\s|$)([\s\S]*)")
...
@dominator_cmd(pattern="rmallfilters$")
