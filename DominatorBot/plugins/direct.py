...
@Dominator_cmd(pattern="direct(?:\s|$)([\s\S]*)")
async def direct_link_generator(request):
    if request.fwd_from:
        return
    dominatorevent = await eor(request, "`Processing...`")
    ...
    if message:
        ...
        await dominatorevent.edit(f"**Usage :** {hl}direct <url>`")
        return
    reply = ""
    links = re.findall(r"\bhttps?://.*\.\S+", message)
    if not links:
        reply = "`No links found!`"
        await dominatorevent.edit(reply)
    for link in links:
