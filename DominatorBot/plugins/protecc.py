...
from DominatorBot.sql.waifu_sql import is_harem, add_grp, rm_grp, get_all_grp
from DominatorBot.sql.husb_sql import is_husb, add_hus_grp, rm_hus_grp, get_all_hus_grp
from . import *
...
@Dominator_cmd(pattern="pt(?:\s|$)([\s\S]*)")
async def _(event):
    BASE_URL = "http://images.google.com"
    if event.reply_to_msg_id:
        dominator = await eor(event, "Hmm..")
        ...
        OUTPUT_STR = """/protecc {prs_text}""".format(
            **locals())
        await dominator.edit(OUTPUT_STR, parse_mode="HTML", link_preview=False)
        ...
        if int(grps.chat_id) == event.chat_id:
            try:
                ...
                try:
                    ...
                    pass
                dominator = await event.client.send_message(event.chat_id, f"/protecc {text}")
                await sleep(2)
...
@Dominator_cmd(pattern="adwaifu(?:\s|$)([\s\S]*)")
async def _(event):
...
@Dominator_cmd(pattern="adhusb(?:\s|$)([\s\S]*)")
async def _(event):
    if not event.is_group:
...
@Dominator_cmd(pattern="rmwaifu(?:\s|$)([\s\S]*)")
async def _(event):
    if not event.is_group:
        await eod(event, "Autowaifu works in groups only !!")
        return
...
@Dominator_cmd(pattern="rmhusb(?:\s|$)([\s\S]*)")
async def _(event):
    if not event.is_group:
        await eod(event, "Autohusbando works in groups only !!")
        return
...
@Dominator_cmd(pattern="aw$")
async def _(event):
    dominator = await eor(event, "Fetching Autowaifu chats...")
    all_grp = get_all_grp()
    ...
    for i in all_grp:
        ...
        cht = int(ch)
        x += f"• `{cht}`\n"
    await dominator.edit(x)


@Dominator_cmd(pattern="ah$")
async def _(event):
    dominator = await eor(event, "Fetching Autohusbando chats...")
    all_grp = get_all_hus_grp()
