...
@dominator_cmd(pattern="ocr(?:\s|$)([\s\S]*)")
async def parse_ocr_space_api(event):
    ...
    try:
        ...
        await eod(dominator, "**Errors !!** \n`{}`\n**Report This to** {}\n\n`{}`".format(
                str(e), dominator_grp, json.dumps(test_file, sort_keys=True, indent=4)
            )
        )
    else:
        await dominator.edit("Read Document in {} seconds. \n{}".format(
                ProcessingTimeInMilliseconds, ParsedText
            )
        )
    os.remove(downloaded_file_name)
    await dominator.edit(ParsedText)
