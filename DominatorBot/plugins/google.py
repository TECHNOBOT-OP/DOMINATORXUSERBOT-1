...
@Dominator_cmd(pattern="wiki(?:\s|$)([\s\S]*)")
async def _(event):
    match = event.text[6:]
    result = None
    try:
...
@Dominator_cmd(pattern="google(?:\s|$)([\s\S]*)")
async def google(event):
    input_str = event.pattern_match.group(1)
    if not input_str:
        return await eod(event, "`Give something to search..`")
    dominator = await eor(event, "Searching...")
    gos = GoogleSearch()
    try:
...
{output}"""
    ...
    await dominator.edit(f"**Google Search For** `{input_str}` \n[📌 See Results Here]({paste})", link_preview=False)


@Dominator_cmd(pattern="img(?:\s|$)([\s\S]*)")
async def img(event):
    sim = event.pattern_match.group(1)
    if not sim:
        return await eod(event, "`Give something to search...`")
    dominator = await eor(event, f"Searching for `{sim}`...")
    if ";" in sim:
        try:
            lim = int(sim.split(";")[1])
            sim = sim.split(";")[0]
...
@Dominator_cmd(pattern="reverse(?:\s|$)([\s\S]*)")
async def _(event):
    ...
    link = alls["href"]
    text = alls.text
    await dominator.edit(f"**Possible Results :** [{text}](google.com{link})")
...
@Dominator_cmd(pattern="gps(?:\s|$)([\s\S]*)")
async def gps(event):
    ...
    dominator = await eor(event, "Finding😁")
    geolocator = Nominatim(user_agent="DominatorBot")
    ...
    if geoloc:
        ...
        await eod(dominator, "I coudn't find it😫")
