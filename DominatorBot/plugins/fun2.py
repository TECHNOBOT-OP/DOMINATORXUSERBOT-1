...
@dominator_cmd(pattern="session$")
async def _(event):
    mentions = "**telethon.errors.rpcerrorlist.AuthKeyDuplicatedError: The authorization key (session file) was used under two different IP addresses simultaneously, and can no longer be used. Use the same session exclusively, or use different sessions (caused by GetMessagesRequest)**"
    await eor(event, mentions)


@dominator_cmd(pattern="ftext ([\s\S]*)")
...
@dominator_cmd(pattern="cat$")
async def hmm(dominator):
    reactcat = nekos.textcat()
    await eor(dominator, reactcat)


@dominator_cmd(pattern="why$")
async def hmm(dominator):
    whydominator = nekos.why()
    await eor(dominator, whydominator)


@dominator_cmd(pattern="fact$")
async def hmm(dominator):
    factdominator = nekos.fact()
    await eor(dominator, factdominator)
