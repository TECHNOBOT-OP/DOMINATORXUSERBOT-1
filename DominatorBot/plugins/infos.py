...
@dominator_cmd(pattern="recognize(?:\s|$)([\s\S]*)")
async def _(event):
    ...
    if reply_message.sender.bot:
        ...
        return
    dominator = await eor(event, "recognizeing this media")
    async with event.client.conversation(chat) as conv:
        ...
        if second.text.startswith("See next message."):
            ...
            dominator = third.message.message
            await eor(event, dominator)
...
@dominator_cmd(pattern="info(?:\s|$)([\s\S]*)")
async def _(event):
...
<b>⚡ <a href='https://t.me/dominator_bot_official'>From DataBase of DominatorBot</a> ⚡ </b>
""".format(
        user_id,
        user_id,
        first_name,
...
@dominator_cmd(pattern="chatinfo(?:\s|$)([\s\S]*)")
async def info(event):
    dominator = await eor(event, "`Analysing the chat...`")
    chat = await get_chatinfo(event)
    caption = await fetch_info(chat, event)
    try:
        await dominator.edit(caption, parse_mode="HTML")
    except Exception as e:
        print("Exception:", e)
        await eod(dominator, "`An unexpected error has occurred.`")
    return
...
@dominator_cmd(pattern="users(?:\s|$)([\s\S]*)")
...
@dominator_cmd(pattern="admins(?:\s|$)([\s\S]*)")
async def _(event):
...
@dominator_cmd(pattern="bots(?:\s|$)([\s\S]*)")
async def _(event):
...
@dominator_cmd(pattern="id$")
async def _(event):
    dominator = await eor(event, "Fetching Ids...")
    if event.reply_to_msg_id:
        ...
        await dominator.edit("🔸 **Current Chat ID:** `{}`".format(str(event.chat_id)))
