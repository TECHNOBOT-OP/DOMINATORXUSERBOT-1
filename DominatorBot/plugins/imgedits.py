...
if not os.path.isdir("./DominatorBot/"):
    os.makedirs("./DominatorBot/")


@Dominator_cmd(pattern="feye$")
async def fun(event):
    path = "omk"
    dominator = await eor(event, "Editing In Progress...")
...
@Dominator_cmd(pattern="warp$")
async def fun(event):
    path = "omk"
    dominator = await eor(event, "Warping In Progress...")
    reply = await event.get_reply_message()
    lol = await event.client.download_media(reply.media, path)
...
@Dominator_cmd(pattern="distort$")
async def fun(event):
    path = "omk"
    dominator = await eor(event, "Distortion In Progress...")
    reply = await event.get_reply_message()
    lol = await event.client.download_media(reply.media, path)
    file_name = "dist.jpg"
    hehe = path + "/" + file_name
