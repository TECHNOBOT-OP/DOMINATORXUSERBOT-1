...
@Dominator_cmd(pattern="stog(?:\s|$)([\s\S]*)")
async def _(event):
    if not event.reply_to_msg_id:
        return await eod(event, "Reply to animated sticker to make gif.")
    dominator = await eor(event, "Converting...")
    if event.pattern_match.group(1):
        quality = event.pattern_match.group(1)
    else:
        quality = 512
    rply = await event.get_reply_message()
    dominator_ = await event.client.download_media(rply.media)
    gifs = tgs_to_gif(dominator_, quality)
    await event.client.send_file(event.chat_id, file=gifs, force_document=False)
    await dominator.delete()


@Dominator_cmd(pattern="stoi$")
async def _(dominator):
    reply_to_id = dominator.message.id
    if dominator.reply_to_msg_id:
        reply_to_id = dominator.reply_to_msg_id
    event = await eor(dominator, "Converting.....")
    ...
    if event.reply_to_msg_id:
        ...
        downloaded_file_name = await dominator.client.download_media(
...
@Dominator_cmd(pattern="ttf(?:\s|$)([\s\S]*)")
async def get(event):
    name = event.text[5:]
    if name is None:
...
@Dominator_cmd(pattern="nfc(?:\s|$)([\s\S]*)")
async def _(event):
    if not event.reply_to_msg_id:
