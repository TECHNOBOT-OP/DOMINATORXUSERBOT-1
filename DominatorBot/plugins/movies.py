...
@dominator_cmd(pattern="imdb(?:\s|$)([\s\S]*)")
async def _(event):
    reply_to = await reply_id(event)
    hel_ = await eor(event, "`Processing ...`")
    try:
        dominator = event.pattern_match.group(1)
        await hel_.edit("__Searching for__ `{}`".format(dominator))
        # Credits to catuserbot.
        # Ported to DominatorBot and beautifications by @ForGo10God.
        movies = imdb.search_movie(dominator)
...
<b>Director :</b> <code>{mov_director}</code>"""
        ...
        paste = await telegraph_paste(f"IMDb Movie Result for “ {dominator} ”", telegraph_)
        ...
        await hel_.edit(f"__No movie found with name {dominator}.__")
    except Exception as e:
        await hel_.edit(f"**Error:**\n__{e}__")


@dominator_cmd(pattern="watch(?:\s|$)([\s\S]*)")
async def _(event):
    query = event.pattern_match.group(1)
    dominator = await eor(event, "Finding Sites...")
