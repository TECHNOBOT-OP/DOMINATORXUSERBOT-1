...
@dominator_cmd(pattern="ls(?:\s|$)([\s\S]*)")
async def lst(event):
...
@dominator_cmd(pattern="ls_local$")
async def _(event):
...
@dominator_cmd(pattern="ls_saved$")
async def _(event):
    PROCESS_RUN_TIME = 100
    cmd = "ls ./SAVED/"
    reply_to_id = event.message.id
...
@dominator_cmd(pattern="rnsaved(?:\s|$)([\s\S]*)")
async def _(event):
    PROCESS_RUN_TIME = 100
    input_str = event.pattern_match.group(1)
    if "|" in input_str:
...
@dominator_cmd(pattern="rnlocal(?:\s|$)([\s\S]*)")
async def _(event):
    PROCESS_RUN_TIME = 100
    input_str = event.pattern_match.group(1)
    if "|" in input_str:
...
@dominator_cmd(pattern="delsave(?:\s|$)([\s\S]*)")
...
@dominator_cmd(pattern="delocal(?:\s|$)([\s\S]*)")
async def handler(event):
    input_str = event.pattern_match.group(1)
...
CmdHelp("managers").add_command(
  "ls_local", None, "Gives the list of downloaded medias in your DominatorBot server."
).add_command(
  "ls_root", None, "Gives the list of all files in root directory of DominatorBot repo."
).add_command(
  "ls_saved", None, "Gives the list of all files in Saved directory of your DominatorBot server"
).add_command(
