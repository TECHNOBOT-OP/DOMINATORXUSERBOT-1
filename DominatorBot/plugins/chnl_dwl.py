...
@dominator_cmd(pattern="getc ([\s\S]*)")
async def get_media(event):
    ...
    dominator = await eor(event, "Downloading Media From this Channel.")
    ...
    await dominator.edit("Downloaded " + output + " files.")


@dominator_cmd(pattern="geta ([\s\S]*)")
async def get_media(event):
    dir = "./channel_dwl/"
    try:
        os.makedirs("./channel_dwl/")
    ...
    print(channel_username)
    dominator = await eor(event, "Downloading All Media From this Channel.")
...
CmdHelp("chnl_dwl").add_command(
  ...
  "getc", "<limit> <channel username>", "will download latest given number of media from channel into your bot server", "getc 1000 @Its_DominatorBot"
).add_info(
