...
@dominator_cmd(pattern="fext ([\s\S]*)")
async def _(event):
...
@dominator_cmd(pattern="pips(?:\s|$)([\s\S]*)")
...
@dominator_cmd(pattern="speed$")
async def _(event):
    dominator = await eor(event, "calculating...")
    PROCESS_RUN_TIME = 100
    cmd = "speedtest-cli"
    eply_to_id = event.message.id
    if event.reply_to_msg_id:
...
CmdHelp("execmod").add_command(
  ...
  "env", None, "Shows Environment veriables of your DominatorBot"
).add_command(
  "speed", None, "Shows server speed of your DominatorBot"
).add_info(
  "Exec Modules."
).add_warning(
  "✅ Harmless Module."
