from . import *
# Credits to @ForGo10God developer of DominatorBot.
# This is my first plugin that I made when I released first DominatorBot.
...
@Dominator_cmd(pattern="mev(?:\s|$)([\s\S]*)")
async def _(event):
    dominator = event.text[5:]
    ...
    troll = await event.client.inline_query("TrollVoiceBot", f"{(deEmojify(dominator))}")
    if troll:
        await event.delete()
...
@Dominator_cmd(pattern="meev(?:\s|$)([\s\S]*)")
async def _(event):
    ...
    if not dominator:
        if event.is_reply:
            ...
            return await eod(event, "`Sir please give some query to search and download it for you..!`")
    troll = await event.client.inline_query("Myinstantsbot", f"{(deEmojify(dominator))}")
    if troll:
        await event.delete()
        hel_ = await troll[0].click(Config.LOGGER_ID)
        if hel_:
