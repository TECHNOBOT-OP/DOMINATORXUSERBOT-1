...
@Dominator_cmd(pattern="restart$")
async def re(dominator):
    event = await eor(dominator, "Restarting Hêllẞø† ...")
    try:
        await restart(event)
...
@Dominator_cmd(pattern="reload$")
async def rel(event):
    await eor(event, "Reloading Hêllẞø†... Wait for few seconds...")
    await reload_DominatorBot()


@Dominator_cmd(pattern="shutdown$")
async def down(dominator):
    event = await eor(dominator, "`Turing Off Hêllẞø†...`")
...
@Dominator_cmd(pattern="svar(?:\s|$)([\s\S]*)")
async def sett(event):
    hel_ = event.pattern_match.group(1)
    var_ = hel_.split(" ")[0].upper()
    val_ = hel_.split(" ")[1:]
    valu = " ".join(val_)
    dominator = await eor(event, f"**Setting variable** `{var_}` **as** `{valu}`")
    if var_ == "":
        return await eod(dominator, f"**Invalid Syntax !!** \n\nTry: `{hl}svar VARIABLE_NAME variable_value`")
    elif valu == "":
        return await eod(dominator, f"**Invalid Syntax !!** \n\nTry: `{hl}svar VARIABLE_NAME variable_value`")
    if var_ not in db_config:
        return await eod(dominator, f"__There isn't any DB variable named__ `{var_}`. __Check spelling or get full list by__ `{hl}vars`")
    try:
        addgvar(var_, valu)
    except Exception as e:
        return await eod(dominator, f"**ERROR !!** \n\n`{e}`")
    await eod(dominator, f"**Variable Added Successfully!!** \n\n**• Variable:** `{var_}` \n**» Value:** `{valu}`")


@Dominator_cmd(pattern="gvar(?:\s|$)([\s\S]*)")
async def gett(event):
    var_ = event.pattern_match.group(1).upper()
    dominator = await eor(event, f"**Getting variable** `{var_}`")
    if var_ == "":
        return await eod(dominator, f"**Invalid Syntax !!** \n\nTry: `{hl}gvar VARIABLE_NAME`")
    if var_ not in db_config:
        return await eod(dominator, f"__There isn't any variable named__ `{var_}`. __Check spelling or get full list by `{hl}vars`")
    try:
        sql_v = gvarstat(var_)
        os_v = os.environ.get(var_) or "None"
    except Exception as e:
        return await eod(dominator, f"**ERROR !!** \n\n`{e}`")
    await dominator.edit(f"**• OS VARIABLE:** `{var_}`\n**» OS VALUE :** `{os_v}`\n------------------\n**• DB VARIABLE:** `{var_}`\n**» DB VALUE :** `{sql_v}`\n")


@Dominator_cmd(pattern="dvar(?:\s|$)([\s\S]*)")
async def dell(event):
    var_ = event.pattern_match.group(1).upper()
    dominator = await eor(event, f"**Deleting Variable** `{var_}`")
    if var_ == "":
        return await eod(dominator, f"**Invalid Syntax !!** \n\nTry: `{hl}dvar VARIABLE_NAME`")
    if var_ not in db_config:
        return await eod(dominator, f"__There isn't any variable named__ `{var_}`. Check spelling or get full list by `{hl}vars`")
    if gvarstat(var_):
        try:
            x = gvarstat(var_)
            delgvar(var_)
            await eod(dominator, f"**Deleted Variable Successfully!!** \n\n**• Variable:** `{var_}` \n**» Value:** `{x}`")
        except Exception as e:
            await eod(dominator, f"**ERROR !!** \n\n`{e}`")
    else:
        await eod(dominator, f"**No variable named** `{var_}`")
   

@Dominator_cmd(pattern="(set|get|del) var(?: |$)(.*)(?: |$)([\s\S]*)")
async def variable(dominator):
    lg_id = Config.LOGGER_ID
    if Config.HEROKU_APP_NAME is not None:
        app = Heroku.app(Config.HEROKU_APP_NAME)
    else:
        return await eor(dominator, "**[ HEROKU ]:**\n__Please setup your__ `HEROKU_APP_NAME`")
    exe = dominator.pattern_match.group(1)
    heroku_var = app.config()
    if exe == "get":
        event = await eor(dominator, "Getting Variable Info...")
        cap = "Logger me chala jaa bsdk."
        capn = "Saved in LOGGER_ID !!"
        try:
            xvar = dominator.pattern_match.group(2).split()[0]
            variable = xvar.upper()
            if variable in db_config:
                return await eod(event, f"This is a SQL based variable. Do `{hl}gvar {variable}` to get variable info.")
            if variable in ("DominatorBot_SESSION", "BOT_TOKEN", "HEROKU_API_KEY"):
                if Config.ABUSE == "ON":
                    await event.client.send_file(dominator.chat_id, cjb, caption=cap)
                    await event.delete()
            ...
            with open("configs.json", "r") as fp:
                result = fp.read()
                if len(result) >= 4096:
                    await dominator.client.send_file(
                        dominator.chat_id,
                        "configs.json",
                        reply_to=dominator.id,
        ...
        event = await eor(dominator, "Setting Heroku Variable...")
        xvar = dominator.pattern_match.group(2)
        if not xvar:
            return await eod(event, f"`{hl}set var <Var Name> <Value>`")
        variable = xvar.upper()
        value = dominator.pattern_match.group(3)
        if not value:
            variable = variable.split()[0]
            try:
                value = dominator.pattern_match.group(2).split()[1]
            except IndexError:
                return await eod(event, f"`{hl}set var <Var Name> <Value>`")
        if variable in db_config:
        ...
        event = await eor(dominator, "Getting info to delete Variable")
        try:
            xvar = dominator.pattern_match.group(2).split()[0]
        except IndexError:
...
@Dominator_cmd(pattern="usage$")
async def dyno_usage(dominator):
    event = await eor(dominator, "`Processing...`")
    useragent = (
    ...
    return await event.edit(
        ...
        f"     ★  `{hours}`**h**  `{minutes}`**m**  "
        f"**|**  `{percentage}`**%**"
        f"\n\n**Owner :** {dominator_mention}"
    )


@Dominator_cmd(pattern="logs$")
async def _(event):
    if (HEROKU_APP_NAME is None) or (HEROKU_API_KEY is None):
        return await eor(event, f"Make Sure Your HEROKU_APP_NAME & HEROKU_API_KEY are filled correct. Visit {dominator_grp} for help.", link_preview=False)
    try:
        Heroku = heroku3.from_key(HEROKU_API_KEY)
        app = Heroku.app(HEROKU_APP_NAME)
    except BaseException:
        return await event.reply(f"Make Sure Your Heroku AppName & API Key are filled correct. Visit {dominator_grp} for help.", link_preview=False)
    cid = await client_id(event)
    dominator_mention = cid[2]
    dominator_data = app.get_log()
    await eor(event, dominator_data, deflink=True, linktext=f"**🗒️ Heroku Logs of 💯 lines. 🗒️**\n\n🌟 **Bot Of :**  {dominator_mention}\n\n🚀** Pasted**  ")
