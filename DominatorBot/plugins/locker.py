...
@Dominator_cmd(pattern="lock(?:\s|$)([\s\S]*)")
async def _(event):
    text = event.text[6:]
    cid = await client_id(event)
    dominator_mention = cid[2]
    dominator = await eor(event, f"Trying to lock `{text}`")
    if text == "":
        return await eod(dominator, "Need something to lock...")
    locker = lock(text)
    if not locker:
        return await eod(dominator, f"**🤐 Invalid lock type:** {text} \nDo `{hl}ltype` to get all lock types.")
    await event.client(EditChatDefaultBannedRightsRequest(event.chat_id, locker))
    await event.client.send_file(event.chat_id, restlo, caption=f"{dominator_mention} Locked `{text}` \n__Cause its Rest Time !!__")
    await dominator.delete()


@Dominator_cmd(pattern="unlock(?:\s|$)([\s\S]*)")
async def _(event):
    text = event.text[8:]
    cid = await client_id(event)
    dominator_mention = cid[2]
    dominator = await eor(event, f"Trying to unlock `{text}`")
    if text == "":
        return await eod(dominator, "Need something to unlock...")
    unlocker = unlock(text)
    if not unlocker:
        return await eod(dominator, f"**🤐 Invalid unlock type:** {text} \nDo `{hl}ltype` to get all unlock types.")
    await event.client(EditChatDefaultBannedRightsRequest(event.chat_id, unlocker))
    await event.client.send_file(event.chat_id, shuru, caption=f"**{dominator_mention} unlocked** `{text}`")
    await dominator.delete()


@Dominator_cmd(pattern="ltype$")
async def _(event):
