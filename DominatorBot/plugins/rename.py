...
@dominator_cmd(pattern="rename(?:\s|$)([\s\S]*)")
async def _(event):
    input_str = event.text[8:]
    if input_str == "":
        return await eod(event, "Give a new file name..")
    dominator = await eor(event, f"Renaming it to `{input_str}`")
    ...
    if event.reply_to_msg_id:
        ...
        if os.path.exists(downloaded_file_name):
            await dominator.edit("Downloaded to `{}` in {} seconds.".format(downloaded_file_name, ms)
            )
        else:
            await eod(dominator, "Error Occurred\n {}".format(input_str))
    else:
        await eod(dominator, f"**Syntax Wrong !!** \n\n• `{hl}rename new file name` as reply to a Telegram file")

@dominator_cmd(pattern="rnupload(?:\s|$)([\s\S]*)")
async def _(event):
    input_str = event.text[10:]
    dominator = await eor(event, f"Renaming to `{input_str}`")
    if not os.path.isdir(Config.TMP_DOWNLOAD_DIRECTORY):
        os.makedirs(Config.TMP_DOWNLOAD_DIRECTORY)
    if event.reply_to_msg_id:
        start = datetime.datetime.now()
        ...
        if os.path.exists(downloaded_file_name):
            ...
            await dominator.edit("Downloaded in {} seconds. Uploaded in {} seconds.".format(ms_one, ms_two))
        else:
            await eod(event, "File Not Found {}".format(input_str))
    else:
        await dominator.edit(f"**Syntax Wrong !!** \n\n• `{hl}rnupload new file name`")

@dominator_cmd(pattern="rnsupload(?:\s|$)([\s\S]*)")
async def _(event):
    dominator = await eor(event, "Rename & Upload as streamable format is in progress...")
        ...
        if os.path.exists(downloaded_file_name):
            ...
            try:
                await event.client.send_file(
                    ...
                    downloaded_file_name,
                    thumb=thumb,
                    caption="reuploaded by DominatorBot",
                ...
                await dominator.edit(event, str(e))
            else:
                end = datetime.datetime.now()
                os.remove(downloaded_file_name)
                ms_two = (end - end_one).seconds
                await dominator.edit("Downloaded in {} seconds. Uploaded in {} seconds.".format(
                        ms_one, ms_two
                    )
                )
        else:
            await eod(dominator, "File Not Found {}".format(input_str))
    else:
