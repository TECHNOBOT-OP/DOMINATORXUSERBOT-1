...
from DominatorBot.sql import pmpermit_sql as pm_sql
from . import *

PM_WARNS = {}
PREV_REPLY_MESSAGE = {}
PM_ON_OFF = Config.PM_PERMIT
dominator_ZERO = "Go get some sleep retard. \n\n**Blocked !!**"


@dominator_cmd(pattern="unblock$")
async def unb(event):
    if event.is_private:
...
if PM_ON_OFF != "DISABLE":
    ...
    async def on_new_private_message(event):
        ...
        dominator_FIRST = "**🔥 Ꭰօʍìղąէօɾβօէ Prîvã†é Sêçürïty Prø†öçõl 🔥**\n\ndominator!! Welcome to {}'s PM. This is an automated message.\n\n{}".format(dominator_mention, CSTM_PMP)
        ...
        if dominator_FIRST == message_text:
            return
    ...
    async def do_pm_permit_action(chat_ids, event):
        ...
        if PM_WARNS[chat_ids] == Config.MAX_SPAM:
            r = await event.reply(dominator_ZERO)
